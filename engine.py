...
class CtaEngine(BaseEngine):
    ...
    def add_strategy(
        self, class_name: str, strategy_name: str, vt_symbol: str, setting: dict) -> None:
        """
        Add a new strategy.
        """
    ...
    def load_strategy_class(self) -> None:
        ...
        """
        # 用于从源代码中加载策略类。首先，使用Path(__file__).parent.joinpath("strategies")
        # 获取第一个路径，然后调用load_strategy_class_from_folder()
        # 方法，从该路径加载策略类，参数为
        # "vnpy_ctastrategy.strategies"。然后，使用Path.cwd().joinpath("strategies")
        # 获取第二个路径，并调用load_strategy_class_from_folder()
        # 方法，从该路径加载策略类，参数为
        # "strategies"。
    ...
    def load_strategy_class_from_folder(self, path: Path, module_name: str = "") -> None:
        ...
        # 用于从指定文件夹中加载策略类。首先，使用glob()
        # 函数从指定文件夹中获取指定后缀的文件，然后调用load_strategy_class_from_module()
        # 方法，从模块中加载策略类。在加载模块时，使用importlib.reload()
        # 函数重载模块，以确保策略文件中的任何修改能够立即生效。然后，使用dir()
        # 函数获取模块中的所有属性，并使用getattr()
        # 函数获取属性值，如果属性值是一个类，并且是CtaTemplate的子类，则将该类添加到self.classes字典中。
